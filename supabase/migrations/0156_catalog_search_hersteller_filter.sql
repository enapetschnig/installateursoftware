-- ============================================================
-- Installateur SuperAPP – Migration 0156
-- catalog_search: Hersteller-FILTER für markentreues Retrieval
-- ------------------------------------------------------------
-- "Alles Hager Automaten" muss Hager-Artikel liefern. Die Trigram-Suche
-- kann das nicht: die Marke steht oft NUR im zusatz-Feld ("HAGER MBN116 …"),
-- die Bezeichnung heißt herstellerspezifisch anders (Hager "LS-Schalter",
-- Schneider "Leitungsschutzschalter") – ein Markenwort in der Query verwässert
-- nur den Score. Deshalb: optionaler Parameter p_hersteller filtert hart auf
-- zusatz ILIKE '<marke>%' (Hersteller = erstes Wort in zusatz, vgl. 0154).
-- Signatur ändert sich → DROP der 3-Parameter-Version (keine Overload-
-- Ambiguität für PostgREST); Aufrufe ohne p_hersteller nutzen den Default.
-- ============================================================
...
declare
  ...
  v_herst text := nullif(trim(p_hersteller), '');
begin
  ...
  with hits as (
    (select ci.*, word_similarity(v_query, ci.search)::real as s
      ...
      where ci.organization_id = v_org
        ...
        and (v_herst is null or ci.zusatz ilike v_herst || '%')
...
revoke all on function public.catalog_search(text, integer, uuid, text) from public;
grant execute on function public.catalog_search(text, integer, uuid, text) to authenticated;
grant execute on function public.catalog_search(text, integer, uuid, text) to service_role;
