-- ============================================================
-- Installateur SuperAPP – Migration 0149
-- catalog_search v2: mehrlieferantenfähig (Katalog-Badge + Filter)
-- ------------------------------------------------------------
-- Für den Katalog-Picker im Dokument-Editor und für Organisationen mit
-- MEHREREN Großhändler-Katalogen (z. B. Sonepar + Rexel) braucht die Suche:
--   • catalog_id + katalog_name im Ergebnis (UI-Badge, kollisionsfreie Keys –
--     gleiche Artikelnummern zweier Händler dürfen sich nicht überschreiben)
--   • optionalen Filter p_catalog_id (Suche in genau einem Katalog)
-- Der Return-Typ ändert sich → DROP + CREATE (CREATE OR REPLACE scheitert
-- bei geänderten OUT-Spalten). Das Frontend fängt das kurze Deploy-Fenster
-- ab (searchCatalog liefert bei error []).
-- Sicherheitsmodell unverändert wie 0148: SECURITY DEFINER mit hartem
-- Org-Filter aus dem JWT des Aufrufers (current_org_id()).
-- ============================================================
