-- ============================================================
-- Installateur SuperAPP – Migration 0146
-- Katalog-RLS: current_org_id() als InitPlan (Timeout-Fix Teil 2)
-- ------------------------------------------------------------
-- current_org_id() ist eine SQL-Funktion mit Query auf memberships.
-- In der Form `organization_id = public.current_org_id()` darf Postgres
-- sie PRO ZEILE auswerten – bei 641k Katalog-Artikeln → statement_timeout.
-- Die Form `organization_id = (select public.current_org_id())` wird als
-- InitPlan EINMAL pro Statement berechnet (offizielles Supabase-Pattern).
--
-- Nur die (großen) Katalog-Tabellen werden umgestellt; die Funktion selbst
-- und alle anderen Policies bleiben unverändert.
-- Zusätzlich filtert catalog_search die Organisation nun explizit über eine
-- einmal berechnete Variable (bessere Pläne, Index (org, artikelnummer)).
-- ============================================================
